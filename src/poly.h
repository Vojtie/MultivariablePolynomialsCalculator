...
/**
 * Tworzy wielomian z tablicy jednomianów.
 * Przejmuje na własność tablicę jednomianów @p monos.
 * @param[in] monos : tablica jednomianów
 * @param[in] count : liczba jednomianów
 */
static inline Poly PolyFromMonos(Mono monos[], size_t count) {
  assert (count != 0 && monos);
...
/**
 * Sprawdza, czy wszystkie jednomiany są tożsamościowo równe zeru.
 * @param[in] count : liczba jednomianów
 * @param[in] monos : tablica jednomianów
 * @return Czy jednomiany są równe zeru?   
 */
static inline bool MonosAreZero(size_t count, Mono monos[]) {
  bool res = true;
  for (size_t i = 0; i < count; i++)
    res &= PolyIsZero(&monos[i].p);
  return res;
}
 ...
 */
static inline bool PolyIsZero(const Poly *p) {
  return (PolyIsCoeff(p) && p->coeff == 0) || (!PolyIsCoeff(p) && MonosAreZero(p->size, p->arr));
}
